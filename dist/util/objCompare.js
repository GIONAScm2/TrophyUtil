...
export function isStandardObj(x) {
...
export function sharedPropsAreEqual(obj1, obj2) {
...
export function updateSharedPropsWithChanges(target, source, trackChanges = false, parentKey = '') {
    const changes = [];
...
export function diffAndUpdateSharedProps(target, source, update = false, parentKey = '') {
    const changes = [];
    const sharedKeys = Object.keys(target).filter(key => key in source);
    sharedKeys.forEach(key => {
        const fullKey = parentKey ? `${parentKey}.${key}` : key;
